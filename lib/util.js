...
const TEMPLATE_HTML_URL = 'https://raw.githubusercontent.com/ajay007e/newman-paralell-execution/main/lib/resources/template.html';
...
const GENERATE_REPORTS_SCRIPT_URL = 'https://raw.githubusercontent.com/ajay007e/newman-paralell-execution/main/lib/resources/_generate_reports_script.py';
...
export const generateReport = async (data) => {
  try {
    // destructuring the input
    const { collectionName, collectionSize } = data;
    // running the script to setup the collection and environment with the pattern
    return await runScript(
      path.resolve("./source/.tmp/resources/generateReports.py"),
...
export const showGenerateReports = () => {
  const htmlReport = chalk.italic.dim(`file://${path.resolve("./source/report.html")}`)
  const csvReport = chalk.italic.dim(`file://${path.resolve("./source/report.csv")}`)
  const consolidatedReport = chalk.italic.dim(`file://${path.resolve("./source/consolidated.html")}`)
