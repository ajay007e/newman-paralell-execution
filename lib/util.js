...
const repositoryUrl = process.env.REPOSITORY_URL; // SSH URL of your Bitbucket repository
const branch = process.env.DEV_BRANCH ? process.env.DEV_BRANCH : 'master'; // Name of the repository branch
const files = JSON.parse(process.env.FILES); // Files to download from the repository
const localRepoPath = 'source/.tmp/temp-repo'; // Local path to clone the repository

const upperLimit = process.env.npm_config_end ? parseInt(process.env.npm_config_end) : -1;
const lowerLimit = process.env.npm_config_start ? parseInt(process.env.npm_config_start) : -1;
const isUpdateRequired = process.env.npm_config_load || false;
const isInitialLoad = process.env.npm_config_initial || false;
const isPatternAvailableInEnv = process.env.npm_config_pattern || false;

const downloadFileFromBitbucket = async (filePath, saveAs) => {
  try {
...
const fetchFromRepository = async () => {
  try {
      ...
      await git(localRepoPath).fetch('origin', branch); // Fetch latest changes from the remote branch
      ...
      await git(localRepoPath).checkout(branch); // Checkout the feature branch again
      await git(localRepoPath).rebase(['master']); // Rebase feature branch with master

      await git(localRepoPath).push('origin', branch, { '--force': null }); // Push changes to remote repository
      await Promise.all(files.map(({ path, saveAs }) => downloadFileFromBitbucket(path, saveAs)));
  } catch (error) {
...
const parallelCollectionRun = (collectionName, allFailures, timeTaken, mpb, counter, done) => {
  ...
  newman
    ...
    .on("done", (err, summary) => {
      if (err || summary.error) {
        ...
        timeTaken[counter] = completed - started;
...
const isEmpty = (value) => {
  return (value == null|| value == undefined || (typeof value === "string" && value.trim().length === 0) || (typeof value === "number" && ( isNaN(value) || value < 0)));
}

const generateRange = () => {
  if(isEmpty(upperLimit) && isEmpty(lowerLimit)) return "";
  else if(isEmpty(upperLimit)) return `${lowerLimit}, ${lowerLimit + 1}`;
  else if(isEmpty(lowerLimit)) return `${upperLimit}, ${upperLimit + 1}`;
  return Array(upperLimit - lowerLimit + 1).fill().map((_, idx) => lowerLimit + idx).join(', ')
}

const createMinSubarrays = (lst) => {
  let groups = [], currentGroup = [], currentSum = 0, maxValue = Math.max(...lst);
...
const generateSequence = (lst) => {
  let result = createMinSubarrays(lst);
  let cumm = result.map(arr => arr.length);

  let index = cumm.reduce((acc, curr) => {
...
const printBestPattern = (collectionName, timeTaken) => {
  const optimumPattern = generateSequence(Object.values(timeTaken))
  console.log("Set environment as ",chalk.bold.cyanBright(`PATTERN_${collectionName.toUpperCase()}='${optimumPattern}'`)," for better perfomrance");
}


export const initializeFileSetups = async () => {
    ...
    // download collections and environments from bitbucket repository
    isUpdateRequired && await fetchFromRepository();
    return {
      status: true
    }
...
export const executePrompt = async () => {
    try {
        ...
        const collectionPattern = process.env[`PATTERN_${path.parse(response.collection).name.toUpperCase()}`]
        const collectionRange = generateRange();
        if((!isPatternAvailableInEnv || isEmpty(collectionPattern)) && isEmpty(collectionRange)) {
          response = {...response, ...await prompt([
            patternPrompt
          ])}
        }else if(isPatternAvailableInEnv) {
          response.pattern = collectionPattern;
        } else {
          response.pattern = collectionRange;
        }
...
export const executeTests =  (stringData) => {
  return new Promise( async (resolve, reject) => {
    ...
    for (let index = 1; index <= collectionSize; index++) {
      commands.push((done) => parallelCollectionRun(collectionName, allFailures, timeTaken, mpb, index, done));
    }
    ...
    async.parallel(commands, (err, results) => {
      ...
      isInitialLoad && printBestPattern(collectionName, timeTaken)
